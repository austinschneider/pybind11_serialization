...
PYBIND11_MODULE(pyABC, m) {
    py::class_<pyABC>(m, "pyABC")
        // Basic method registration
        .def(py::init<>())
        .def("method", &ABC::method)
        .def("call", &ABC::call)
        .def("get_self", &pyABC::get_self)
        .def(py::pickle(
            [](const pyABC & cpp_obj) {
                std::cout << "pyABC serialization method" << std::endl;
                py::object self;
                if(cpp_obj.self) {
            ...
            [](const py::tuple &t) {
                std::cout << "pyABC deserialization method" << std::endl;
                if (t.size() != 1) {
                    throw std::runtime_error("Invalid state!");
                }
                auto cpp_state = std::unique_ptr<pyABC>(new pyABC);
    ...
    py::class_<ABC, std::shared_ptr<ABC>, pyABC>(m, "ABC")
        ...
        .def(py::pickle(
            ...
            [](const py::tuple &t) {
                std::cout << "ABC deserialization method" << std::endl;
                if (t.size() != 1) {
                    throw std::runtime_error("Invalid state!");
                }
